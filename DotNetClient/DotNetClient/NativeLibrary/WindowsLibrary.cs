...
{
    public sealed class WindowsLibrary : INativeLibrary
    {
        private const string CPP_LIBRARY = "../CppLibrary/x64/Debug/CppLibrary.dll";

        // Native implementation of GetOne() for Windows
        [DllImport(CPP_LIBRARY)]
        private static extern int get_one();

        // Native implementation of AddTwoNumbers() for Windows
        [DllImport(CPP_LIBRARY)]
        private static extern int add_two_numbers(int i, int j);

        // Native implementation of ExternalCall() for Windows
        [DllImport(CPP_LIBRARY)]
        private static extern int external_call(Functions.AddTwoIntsDelegate func, int i, int j);

        // Native implementation of ExternalReduce() for Windows
        [DllImport(CPP_LIBRARY)]
        private static extern int external_reduce(Functions.ReduceIntArrayDelegate reduceFunc, int[] array, int arraySize);

        // Native implementation of ExternalReduceWithCallback() for Windows
        [DllImport(CPP_LIBRARY)]
